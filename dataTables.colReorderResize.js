 ...
 *
 * Bill Miller, Aug 2016
 * added resizable column functionality to this plugin totally based on 
 * Martin Marchetta's changes for the pre v1.1 version of Datatables.net
 *
 * you can disable the column reordering or resizing at init
 *		colReorder.allowReorder: false,

 *		colReorder.allowResize: false,
...
$.extend( ColReorder.prototype, {
	...
	"_fnMouseListener": function ( i, nTh )
	{
		...
		if (this.s.allowResize) {
			...
			$(nTh).bind('mousemove.ColReorder', function (e) {
				...
				if (that.dom.drag === null && that.dom.resize === null) {
					...
					if (Math.abs(e.pageX - Math.round(offset.left + nLength)) <= 7) {
		...
		$(nTh).on( 'mousedown.ColReorder', function (e) {
			e.preventDefault();
			//bmiller that._fnMouseDown.call( that, e, nTh );
			that._fnMouseDown.call(that, e, nTh, i); //Martin Marchetta: added the index of the column dragged or resized //bmiller
	 ...
	 *  @param   element nTh TH element to be dragged
 	 *  @param 	 i The column that's resized/dragged //bmiller
	...
	"_fnMouseDown": function ( e, nTh, i )
	{
		...
		$(document)
			.on( 'mousemove.ColReorder', function (e) {
				that._fnMouseMove.call( that, e, i );  //Martin Marchetta: Added index of the call being dragged or resized //bmiller
			} )
			.on( 'mouseup.ColReorder', function (e) {
				//bmiller -- this works great, don't know why...
				...
				//Martin Marchetta: Added index of the call being dragged or resized
	 ...
	 *  @method  _fnMouseMove
	 *  @param   event e Mouse event
	 *  @param   colResized Index of the column that's being dragged or resized (index within the internal model, not the visible order) //bmiller
	...
	"_fnMouseMove": function ( e, colResized)
	{
		...
		if(this.table_size < 0 && scrollXEnabled && $('div.dataTables_scrollHead', this.s.dt.nTableWrapper) != undefined){
			...
			if($('div.dataTables_scrollHead', this.s.dt.nTableWrapper).length > 0)
				...
				this.table_size = $('div.dataTables_scrollHead>div>table', this.s.dt.nTableWrapper).width();
		...
		if (this.dom.resize) {       
			...
			if(scrollXEnabled && $('div.dataTables_scrollHead', this.s.dt.nTableWrapper) != undefined){
				...
				if($('div.dataTables_scrollHead', this.s.dt.nTableWrapper).length > 0)
					...
					$('div.dataTables_scrollHead>div>table', this.s.dt.nTableWrapper).width(this.table_size + moveLength);
			...
			if($('div.dataTables_scrollBody') != null){
				...
				if($('div.dataTables_scrollBody').length > 0){
					...
					var scrollingTableHeadRow = $('div.dataTables_scrollHeadInner>table>thead>tr', this.s.dt.nTableWrapper)[0];
					...
					if (moveLength != 0 && !scrollXEnabled){
						...
						$(scrollingTableHeadRow.childNodes[visibleColumnIndex + 1]).width(this.s.mouse.nextStartWidth - moveLength);

					}

					$(scrollingTableHeadRow.childNodes[visibleColumnIndex]).width(this.s.mouse.startWidth + moveLength);

				

					//Get the main table and resize it too, or just the next col if !scrollXEnabled

					if(scrollXEnabled)

						$(tableMain).width(this.table_size + moveLength);
	 ...
	 *  @method  _fnMouseUp
	 *  @param   event e Mouse event
	 *  @param colResized The index of the column that was just dragged or resized (index within the internal model, not the visible order). //bmiller
	...
	"_fnMouseUp": function ( e, colResized)
	{
		...
		//Martin Marchetta
			...
			//bmiller lets just reset this - in case the real width is changed elsewhere
